...
static const t_f	g_testee2 = &ps_stream_internal_node_vec_remove;
...
void	test1(void)
{
	...
	assert(node.count == 2);
	assert(!g_testee2(vec, PS_STREAM_INTERNAL_COMMAND_R));
	assert(!g_testee2(vec, PS_STREAM_INTERNAL_COMMAND_RR));
	assert(g_testee2(vec, PS_STREAM_INTERNAL_COMMAND_S));
	assert(vec->v->get(vec, 0, &node));
	assert(node.command == PS_STREAM_INTERNAL_COMMAND_S);
	assert(node.count == 1);
	assert(!g_testee2(vec, PS_STREAM_INTERNAL_COMMAND_R));
